...
describe('Check AA', function () {
	...
	it('Deploy AA with greater-than comparaison', async () => {
		...
		this.gt_expiry_date = new Date(time_now.getTime() + 10 * 24 * 3600 * 1000)
		this.last_expiry_date = this.gt_expiry_date;
	...
	it('Deploy AA with not-equal comparaison', async () => {
		...
		this.ne_value = 'bar'
		this.ne_operator = '!='
		this.ne_expiry_date = new Date(this.last_expiry_date.getTime() + 30 * 24 * 3600 * 1000)
		this.last_expiry_date = this.ne_expiry_date;
		const { address, unit, error } = await this.network.deployer.deployAgent(`[
			"autonomous agent",
			{
				"base_aa": "${this.network.agent.contract_base}",
				"params": {
						...
						"expiry_date": "${this.ne_expiry_date.toISOString().slice(0, -14)}"
				}
			}
		]`)
		expect(error).to.be.null
	...
	it('Alice tries to flag not-equal-aa yes without any AA posting ', async () => {
		var { unit, error } = await this.network.wallet.alice.triggerAaWithData({
			toAddress: this.ne_aa,
			amount: 10000,
			data: {
	...
	it('Alice tries to flag not-equal-aa yes after oracle posts ', async () => {
		const datafeed = {}
	...
	it('Deploy AA with less-than comparaison', async () => {
		var { error } = await this.network.timetravel({ to: this.last_expiry_date })
		expect(error).to.be.null

		this.last_expiry_date 
		this.lt_feed_name = 'EUR_USD'
		this.lt_value = 1.2
		this.lt_operator = '<'
		this.lt_days_for_expiration = 10
		this.lt_expiry_date = new Date(this.last_expiry_date.getTime() + this.lt_days_for_expiration * 24 * 3600 * 1000).toISOString()
		this.last_expiry_date = this.lt_expiry_date;

		var { address, unit, error } = await this.network.deployer.deployAgent(`[
			"autonomous agent",
			{
					"base_aa": "${this.network.agent.contract_base}",
					"params": {
							"oracle_address": "${await this.network.wallet.oracle.getAddress()}",
							"feed_name":"${this.lt_feed_name}",
							"comparison":"${this.lt_operator}",
							"feed_value":"${this.lt_value}",
							"expiry_date": "${this.lt_expiry_date.slice(0, -14)}"
					}
			}
		]`)
		expect(error).to.be.null
	...
	it('bob converts bytes to less-than asset', async () => {
		this.lt_asset_amount_bob = 196333
		const { unit, error } = await this.network.wallet.bob.sendBytes({
			toAddress: this.lt_aa,
			amount: this.lt_asset_amount_bob
		})
		...
		await this.network.witnessUntilStable(response.response_unit)
		const { vars } = await this.network.deployer.readAAStateVars(this.lt_aa)

		expect(vars.no_asset).to.be.to.be.validUnit
		expect(vars.yes_asset).to.be.to.be.validUnit

		this.lt_yes_asset = vars.yes_asset
		this.lt_no_asset = vars.no_asset

		this.lt_asset_amount_bob -= byte_to_asset_fees
		var bob_balances = await this.network.wallet.bob.getBalance()
		expect(bob_balances[this.lt_yes_asset].stable).to.be.equal(this.lt_asset_amount_bob)
		expect(bob_balances[this.lt_no_asset].stable).to.be.equal(this.lt_asset_amount_bob)
	}).timeout(600000)


	it('bob tries to redeem with no before expiry', async () => {

		var datafeed = {}
		datafeed[this.lt_feed_name] = (this.lt_value).toString()
		...
		var { error } = await this.network.timetravel({ shift: (this.lt_days_for_expiration - 1) + 'd' })
		expect(error).to.be.null

		var { unit, error } = await this.network.wallet.bob.sendMulti({
			asset: this.lt_no_asset,
			asset_outputs: [{
				address: this.lt_aa,
				amount: this.lt_asset_amount_bob
			}],
	...
	it('bob redeems with no', async () => {
		var { error } = await this.network.timetravel({ shift: '2d' })
		expect(error).to.be.null
